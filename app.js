...
(function () {
  ...
  var Store = {
    ...
    _s: function(k, v) { try { localStorage.setItem(STORAGE_PFX + k, JSON.stringify(v)); } catch(e) {} },
  ...
  function showAddToPlaylist(sectionId, chapterId) {
    ...
    var addBtn = h('button', { class: 'playlist-add-btn', text: 'Create', on: { click: function() {
      ...
      if (!name) return;
      playlists.push({ name: name, items: [{ k: key, s: sectionId, c: chapterId, title: title }], created: Date.now() });
      Store.savePlaylists(playlists);
      modal.remove();
    }}});
  ...
  function renderPlaylistsPanel() {
    ...
    var addBtn = h('button', { class: 'playlist-add-btn', text: 'Create', on: { click: function() {
      var name = inp.value.trim();
      if (!name) return;
      playlists.push({ name: name, items: [], created: Date.now() });
      Store.savePlaylists(playlists);
    ...
    if (!playlists.length) { body.appendChild(h('div', { class: 'panel-empty', text: 'No playlists yet. Create one to organize your study.' })); return; }

    playlists.forEach(function(pl, plIdx) {
